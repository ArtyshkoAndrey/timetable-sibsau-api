...
class TeacherController extends Controller
{
  ...
  public function show($teacher_id) {
    $lessons = Lesson::where('teacher_id', '=', $teacher_id)->get();
    ...
    return  response()
      ->json([
        'group' => Teacher::where('id', $teacher_id)->first()->full_name,
        'timetable' => $days,
        'exams' => $exams
      ])
  ...
  public function searchTeacher ($text, $count) {
    // dd('%'.$text.'%');
    $teachers = Teacher::where('full_name', 'LIKE','%'.$text.'%')->take($count)->get();
    return  response()
      ->json([
        'teacher' => $teachers->toArray(),
      ])
      ->header('Content-Type', 'application/json')
      ->header('charset', 'utf-8');
  }
